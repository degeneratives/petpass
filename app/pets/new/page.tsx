  ...
  const handleSubmit = async (e: React.FormEvent, addAnother = false) => {
    ...
    try {
      ...
      if (addAnother) {
        ...
        setTravelHistory('');
        setPrivacy('public');
        alert('Pet profile created! Add another pet.');
      } else {
        router.push(`/pets/${newPet.petId}`);
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="border-b-4 border-primary bg-card">
        <div className="max-w-4xl mx-auto px-4 py-6">
          <Link href="/dashboard" className="inline-flex items-center gap-2 mb-4 text-primary hover:underline font-mono text-sm">
            ← Back to Dashboard
          </Link>
          <h1 className="font-serif text-4xl font-bold text-primary">Create Pet Profile</h1>
          ...
          <Card className="border-2 border-primary mt-8">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                >
                  {saving && !saveAndAddAnother ? 'Creating...' : 'Create Pet Profile →'}
                </Button>
                <Button
                  type="button"
                  ...
                  disabled={saving}
                  variant="outline"
                  className="flex-1 border-2 border-primary font-mono text-xs sm:text-sm uppercase"
                >
